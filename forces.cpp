...
void compute_forces(vector<PARTICLE>& particle, double lj_epsilon, double r_c, double L)
{
    ...
    for (unsigned int i = 0; i < particle.size(); i++)
    {
        ...
        for (unsigned int j = 0; j < particle.size(); j++)
        {
            ...
            if (distance_ij.y < -L/2)
                distance_ij.y = distance_ij.y + L;
            if (distance_ij.z > L/2)
                distance_ij.z = distance_ij.z - L;
            if (distance_ij.z < -L/2)
                distance_ij.z = distance_ij.z + L;
